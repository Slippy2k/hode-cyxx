...
static uint32_t yuv_diff(uint32_t x, uint32_t y, const uint8_t *p) {
	if (1) {
		x = p[x];
		y = p[y];
...
#define df(A, B) yuv_diff(A, B, src)
...
static void scaleImpl(uint32_t *dst, int dstPitch, const uint8_t *src, int srcPitch, int w, int h, const uint32_t *palette) {
	...
	for (int y = 0; y < h; ++y) {
		...
		for (int x = 0; x < w; ++x) {
            ...
            const uint32_t B1 = &sa0[2] - src;
            const uint32_t PB = &sa1[2] - src;
            const uint32_t PE = &sa2[2] - src;
            const uint32_t PH = &sa3[2] - src;
            const uint32_t H5 = &sa4[2] - src;

            const int pprev = 2 - (x > 0);
            const uint32_t A1 = &sa0[pprev] - src;
            const uint32_t PA = &sa1[pprev] - src;
            const uint32_t PD = &sa2[pprev] - src;
            const uint32_t PG = &sa3[pprev] - src;
            const uint32_t G5 = &sa4[pprev] - src;

            const int pprev2 = pprev - (x > 1);
            const uint32_t A0 = &sa1[pprev2] - src;
            const uint32_t D0 = &sa2[pprev2] - src;
            const uint32_t G0 = &sa3[pprev2] - src;

            const int pnext = 3 - (x == w - 1);
            const uint32_t C1 = &sa0[pnext] - src;
            const uint32_t PC = &sa1[pnext] - src;
            const uint32_t PF = &sa2[pnext] - src;
            const uint32_t PI = &sa3[pnext] - src;
            const uint32_t I5 = &sa4[pnext] - src;

            const int pnext2 = pnext + 1 - (x >= w - 2);
            const uint32_t C4 = &sa1[pnext2] - src;
            const uint32_t F4 = &sa2[pnext2] - src;
            const uint32_t I4 = &sa3[pnext2] - src;
...
static void palette_xbr(const uint32_t *palette) {
	for (int i = 0; i < 256; ++i) {
		...
		_yuv[i * 3]     = ( 299 * r + 587 * g + 114 * b) * 4 / 1000;
		_yuv[i * 3 + 1] = (-169 * r - 331 * g + 500 * b) * 4 / 1000 + 128;
		_yuv[i * 3 + 2] = ( 500 * r - 419 * g -  81 * b) * 4 / 1000 + 128;
	}
}
