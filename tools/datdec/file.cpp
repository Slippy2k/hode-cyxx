...
struct File_impl {
	...
	virtual void seek(int32_t off, int whence) = 0;
	virtual void read(void *ptr, uint32_t len) = 0;
	virtual void write(const void *ptr, uint32_t len) = 0;
};
...
struct stdFile : File_impl {
	...
	void write(const void *ptr, uint32_t len) {
		if (_fp) {
...
void File::write(const void *ptr, uint32_t len) {
	_impl->write(ptr, len);
}
